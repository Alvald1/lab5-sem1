...
void
gnome_sort(Node** arr, int n, fptr_compare compare, int flag) {
    int index = 0;
    while (index < n) {
        if (index == 0) {
            index++;
...
void
insertion_sort(Node** arr, int n, fptr_compare compare, int flag) {
    int i, j;
    Node* key;
    for (i = 1; i < n; i++) {
        key = *(arr + i);
...
void
_qsort(Node** arr, int left, int right, fptr_compare compare, int flag) {
    int i, last;
    if (left >= right) {
        return;
    }
...
int
cmp_name(Node* node_1, Node* node_2) {
    return strcmp(node_1->name, node_2->name);
}

int
cmp_id(Node* node_1, Node* node_2) {
    return strcmp(node_1->id, node_2->id);
}

int
cmp_time(Node* node_1, Node* node_2) {
