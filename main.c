        ...
        switch (state) {
            ...
            case 'b':
                ...
                if (scanf("%c", &state) != EOF) {
                    scanf("%*c");
                    switch (state) {
                        case 'a': write(stdout, nodes, len, FILE_MODE); break;
                        case 'b':
                            file_name = readline(stdin, "Введите название файла\n");
                ...
                if (scanf("%c", &state) != EOF) {
                    ...
                    if (scanf("%c", &state_2) != EOF) {
                        ...
                        if (scanf("%c", &state_3) != EOF) {
                            scanf("%*c");
                            switch (state_2) {
                                case 'a': compare = cmp_name; break;
                                case 'b': compare = cmp_id; break;
                                case 'c': compare = cmp_time; break;
                            }
                            switch (state_3) {
                                case 'a': dir = DIRECT; break;
                                case 'b': dir = REVERSE; break;
                            }
                            switch (state) {
                                case 'a': _qsort(nodes, 0, len - 1, compare, dir); break;
                                case 'b': gnome_sort(nodes, len, compare, dir); break;
                                case 'c': insertion_sort(nodes, len, compare, dir); break;
                            }
                        }
                    } else {
                        return 0;
                        ...
                        if (scanf("%c", &state_3) != EOF) {
                            ...
                            if (scanf("%d", &len) != EOF) {
                                ...
                                if (scanf("%d", &cnt) != EOF) {
                                    scanf("%*c");
                                    switch (state_2) {
                                        case 'a': cmp = NAME_MODE; break;
                                        case 'b': cmp = ID_MODE; break;
                                        case 'c': cmp = TIME_MODE; break;
                                    }
                                    switch (state_3) {
                                        case 'a': dir = DIRECT; break;
                                        case 'b': dir = REVERSE; break;
                                    ...
                                    switch (state) {
                                        case 'a': sort = QSORT_MODE; break;
                                        case 'b': sort = GNOME_MODE; break;
                                        case 'c': sort = INSERTION_MODE; break;
                                    }
